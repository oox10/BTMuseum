  ...
  */
  ...
  class ImportDobj{
	...
	public function __construct($db,$task=0){
	  ...
	  $this->PdfProcessLib = _SYSTEM_ROOT_PATH.'mvc/lib/gs9.22/bin/gswin64c.exe -dNOPAUSE -dBATCH -sDEVICE=jpeg ';
	...
	protected function prepareImport($UplRecord){
	  ...
	  try{ 
		...
		$this->TaskData['dotype']   = $UplRecord['dotag'] ? $UplRecord['dotag'] : '';
		
		// 檢測資料夾檔案，決定命名
		$save_folder = $this->TaskData['saveroot'].'saved/'.$this->TaskData['folder'].'/';
		
		if(!preg_match('/^'.$this->TaskData['folder'].'-\d{3}\..*/',$UplRecord['name'])){
		
    		$dono_max = 0;
		
			if(is_dir($save_folder)){
			  $dofiles = array_slice(scandir($save_folder),2);	
			  foreach($dofiles as $dof){
				$dono = intval(preg_replace('/^'.$this->TaskData['folder'].'-/','',$dof));	
				$dono_max = ($dono > $dono_max) ? $dono : $dono_max;
			  }
			}
			$this->TaskData['finename'] = $UplRecord['folder'].'-'.str_pad(($dono_max+1),3,'0',STR_PAD_LEFT).'.'.strtolower($UplRecord['type']);
		}else{
	...
	private function activeImport(){
	  ...
	  try{
		...
		switch($this->UploData['mime']){
			case 'image/jpeg': $imgprocess[$this->TaskData['finename']] = self::process_image( 'jpg'  , $this->TaskData['source'],$this->TaskData['saveroot'],$this->TaskData['folder'],$this->TaskData['finename']); break;
			case 'image/png' : $imgprocess[$this->TaskData['finename']] = self::process_image( 'png'  , $this->TaskData['source'],$this->TaskData['saveroot'],$this->TaskData['folder'],$this->TaskData['finename']); break;
			case 'image/raf': $imgprocess[$this->TaskData['finename']] = self::process_image('raf',$this->TaskData['source'],$this->TaskData['saveroot'],$this->TaskData['folder'],$this->TaskData['finename']); break;
			case 'image/cr2':
			case 'image/x-canon-cr2':$imgprocess[$this->TaskData['finename']] = self::process_image('cr2',$this->TaskData['source'],$this->TaskData['saveroot'],$this->TaskData['folder'],$this->TaskData['finename']); break;
			case 'image/tiff': 
			  if($this->UploData['type']=='dng'){
				$imgprocess[$this->TaskData['finename']] = self::process_image( 'dng' , $this->TaskData['source'],$this->TaskData['saveroot'],$this->TaskData['folder'],$this->TaskData['finename']);   
			  }else{
				$imgprocess[$this->TaskData['finename']] = self::process_image( 'tif' , $this->TaskData['source'],$this->TaskData['saveroot'],$this->TaskData['folder'],$this->TaskData['finename']);   
			  }  
			  break;
			case 'image/dng': $imgprocess[$this->TaskData['finename']] = self::process_image('dng',$this->TaskData['source'],$this->TaskData['saveroot'],$this->TaskData['folder'],$this->TaskData['finename']); break;  
			case 'application/octet-stream':  // 
			  switch($this->UploData['type']){
			    case 'cr2':$imgprocess[$this->TaskData['finename']] = self::process_image('cr2',$this->TaskData['source'],$this->TaskData['saveroot'],$this->TaskData['folder'],$this->TaskData['finename']); break;
				case 'dng':$imgprocess[$this->TaskData['finename']] = self::process_image('dng',$this->TaskData['source'],$this->TaskData['saveroot'],$this->TaskData['folder'],$this->TaskData['finename']); break;
				case 'raf':$imgprocess[$this->TaskData['finename']] = self::process_image('raf',$this->TaskData['source'],$this->TaskData['saveroot'],$this->TaskData['folder'],$this->TaskData['finename']); break;
			    default:break;
			  }
			  
			case 'application/pdf':  // 
			  $imgprocess = self::process_pdf($this->TaskData['source'],$this->TaskData['saveroot'],$this->TaskData['folder'],$this->TaskData['finename']); break;
			  break;
		...
		// 填入DO profile
		$rewrite = 0;
		foreach($imgprocess as $img_file => $presult){
			$img_size = explode(' x ',$presult['s']);
			$newfileconf = [
			  'file'=>$img_file,
			  'width'=> intval($img_size[0]),
			  'height'=> intval($img_size[1]),
			  'size'=>filesize($this->TaskData['saveroot'].'browse/'.$this->TaskData['folder'].'/'.$img_file),
			  'dotype'=>$this->TaskData['dotype']
			];
			foreach($this->FolderConf['items'] as $i=>$dof ){
			  if( $dof['file'] == $img_file ){
	...
	//-- 處理PDF檔案
	public function process_pdf( $FileSource , $FileRoot , $MainFolder, $FileName){
	  ...
	  $file_package = $FileRoot.'package/'.$MainFolder.'/'; // 原始檔案位置 
	  ...
	  if(!is_dir($file_package))  mkdir($file_package,0777,true);  
	  
	  $pdf_savename = $MainFolder.'.pdf';  // 檔案最後存放位置
	  
	  //確認原始PDF資料是否有重複 //僅處理原始檔案
	  if(file_exists($file_package.$pdf_savename)) copy($file_package.$pdf_savename,$FileRoot.'trash/'.$pdf_savename.'-'.microtime(true));
	  
	  //移動打包檔案
	  copy($FileSource,$file_package.$pdf_savename);
      
	  //清空資料夾 
	  $old_imgs = array_slice(scandir($file_store),2);
	  foreach($old_imgs as $img){
		unlink($file_store.$img);  
		unlink($file_browse.$img);  
		unlink($file_thumb.$img);  
	  }
	  
	  //extract image
	  $config = ' -sOutputFile="'.$file_store.$MainFolder.'-%04d.jpg" -r300x300 ';
	  exec($this->PdfProcessLib.$config.'"'.$file_package.$pdf_savename.'"',$output,$result);
	  if(count($output)) file_put_contents("process.log",print_r($output, true),FILE_APPEND | LOCK_EX);	
	   
	  $new_imgs = array_slice(scandir($file_store),2);
	  foreach($new_imgs as $img){
		$imgprocess[$img] = [];
		list($width, $height, $type, $attr) = getimagesize($file_store.$img);  
	    $imgprocess[$img]['o']  = $width.' x '.$height;
	    $imgprocess[$img]['s']  = self::image_resize($file_store.$img,$file_browse.$img,'m');  // 處理系統圖片
	    $imgprocess[$img]['t']  = self::image_resize($file_store.$img,$file_thumb.$img,'s');    // 處理縮圖 
	  }
	...
	public function process_image($itype , $FileSource , $FileRoot , $MainFolder, $FileName){
	  ...
	  //確認原始資料是否有重複 //僅處理原始檔案
	  if(file_exists($file_store.$FileName)) copy($file_store.$FileName,$FileRoot.'trash/'.$FileName.'-'.microtime(true));
	...
	public function image_resize($filesource,$filesave,$size){	
		...
		if($size == 'm'){  
		  if( $width > 1190 || $height > 1684 ){
			$config = ($width >= $height) ?  ' -resize 1190x ' : ' -resize x1684 ';
		    $config.= '-quality 70 ';
