...
*/
  ...
  **************************************/
  ...
  function active_loading( domObject , status){
	var loadmark = '<i class="loader"><div style="width:100%;height:100%" class="lds-dual-ring"><div></div></div></i>';
	if(!domObject.parent('.processing').length){
	  domObject.wrap( "<div class='processing' status='L'></div>" );
	  domObject.parent('.processing').append(loadmark+'<i class="fa fa-check" aria-hidden="true"></i><i class="fa fa-exclamation-triangle" aria-hidden="true"></i>');
	}
	var processing = domObject.parent('.processing');
	processing.children().finish();
	switch(status){
	  case false   : case 0   : 
	    processing.attr('status','F').find('i.fa-exclamation-triangle').effect("bounce", "slow" ).delay( 1000 ).animate({opacity:0},2000,function(){ processing.children("i").remove(); domObject.unwrap(); }); ;
		domObject.prop('disabled',false);
	    break;
	  case true    : case 1   : 
	    processing.attr('status','Y').find('i.fa-check').animate({opacity:0},2000,function(){ processing.children("i").remove(); domObject.unwrap(); }); 
	    domObject.prop('disabled',false);
		break;
	  case 'initial': processing.attr('status','L'); domObject.prop('disabled','true'); break;
	}
  }
